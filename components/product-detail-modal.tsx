...
export default function ProductDetailModal({ isOpen, onClose, product }: ProductDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-md transition-all md:p-8 overflow-y-auto">
      {/* Container forcing a Dark Theme Aesthetic */}
      <div className="bg-[#0a0a0a] text-zinc-100 rounded-lg max-w-[1000px] w-full relative shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-zinc-800/50 my-auto flex flex-col mt-[15vh] md:mt-auto">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 text-zinc-400 hover:text-orange-500 transition-colors text-3xl font-light w-10 h-10 flex items-center justify-center rounded-full hover:bg-zinc-900 bg-black/50 shadow-sm md:shadow-none md:bg-transparent"
        >
          &times;
        </button>

        <div className="grid md:grid-cols-12 md:max-h-[85vh]">
          {/* Left Box: Product Images Section */}
          <div className="md:col-span-7 flex flex-col md:flex-row bg-[#111111] p-6 lg:p-10 gap-6 border-r border-zinc-800/50 rounded-l-lg">
            {/* Vertical Thumbnails */}
            {uniqueImages.length > 1 && (
              <div className="flex md:flex-col gap-3 overflow-x-auto md:overflow-y-auto md:h-[450px] hide-scrollbar pb-2 md:pb-0 w-full md:w-20 shrink-0 order-2 md:order-1 pt-4 md:pt-0">
                {uniqueImages.map((img, i) => (
                  <button
                    key={i}
                    onClick={() => setMainImage(img as string)}
                    className={`relative w-16 h-16 md:w-16 md:h-16 lg:w-20 lg:h-20 rounded-md bg-[#1a1a1a] flex-shrink-0 transition-all ${
                      mainImage === img 
            ...
            <div className="flex-1 overflow-hidden relative flex items-center justify-center order-1 md:order-2 bg-[#111111] rounded-lg">
              <img
                src={mainImage}
                alt={product.name}
                className="w-full h-auto max-h-[450px] object-contain drop-shadow-2xl transition-transform duration-500 hover:scale-105"
              />
              {displayBadge && (
                <div className={`absolute top-4 right-4 px-3 py-1.5 rounded-sm text-xs font-bold tracking-widest text-white ${
          ...
          <div className="md:col-span-5 flex flex-col p-6 lg:p-10 bg-[#0a0a0a] rounded-r-lg">
            ...
            <div className="flex w-full mb-8 border-b border-zinc-800">
              {(['gallery', 'description', 'information'] as const).map((tab) => (
                <button
                  ...
                  className={`flex-1 pb-4 text-[11px] lg:text-xs font-semibold tracking-[0.14em] uppercase transition-colors ${
                    activeTab === tab 
            ...
            <div className="flex-1 overflow-y-auto hide-scrollbar">
              ...
              <div className="mb-6 md:mb-8">
                <div className="text-zinc-500 text-[10px] font-sans tracking-widest mb-2 flex justify-between uppercase">
                  <span>BIZIM BAGS</span>
                  <div className="flex items-center gap-1 text-orange-500">
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                    <span className="font-bold text-zinc-300">{product.rating ? product.rating.toFixed(1) : '5.0'}</span>
                  </div>
                </div>
                <h1 className="text-2xl md:text-3xl font-heading text-white mb-2 leading-tight">
                ...
                <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
                  ...
                  {colors.length > 0 && (
                    <div className="space-y-4">
                      <div className="flex justify-between items-center text-[10px] tracking-widest uppercase font-semibold text-zinc-400">
                      ...
                      <div className="flex flex-wrap gap-4 pt-1">
                        {colors.map((c: any, i: number) => {
                          ...
                          return (
                            ...
                            >
                              {isSelected && (
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={hex === '#FFFFFF' || hex === '#F8F9FA' ? '#000' : '#FFF'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <div className="space-y-0 text-sm">
                    <div className="flex justify-between items-end border-b border-zinc-800 pb-2 mb-2">
                      <h3 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-[0.1em]">Sizes</h3>
                      <h3 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-[0.1em]">Item Price & Quantity</h3>
                    </div>
                    
                    <div className="flex items-center justify-between py-4 border-b border-zinc-800/50">
                      <div>
                        <span className="text-zinc-200 font-medium">Standard</span>
                        <div className="text-[9px] text-zinc-500 font-mono mt-1">SKU: {product.id.toString().slice(0,8).padStart(8,'0')}</div>
                      </div>
                      
                      <div className="flex items-center gap-3 md:gap-5">
                        {/* Custom Counter Input */}
                        <div className="flex items-center border border-zinc-700 rounded-sm bg-[#111111] h-9">
                          <button
                            onClick={() => setQuantity(Math.max(1, quantity - 1))}
                            className="w-8 h-full flex items-center justify-center text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors disabled:opacity-30"
                          ...
                          <input
                            ...
                            className="w-8 text-center bg-transparent text-sm font-semibold text-white focus:outline-none hide-arrows py-0"
                            disabled={stockLimit === 0}
                          />
                          <button
                        ...
                        <div className="text-right w-[75px] md:w-[85px]">
                          <span className="text-[15px] md:text-[17px] font-bold text-white whitespace-nowrap">
                  ...
                  <div className="flex justify-between items-end pt-2 pb-1">
                    <h3 className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest">Total</h3>
                    <span className="text-2xl md:text-3xl font-heading text-orange-500 font-normal">
