...
export default function Header() {
  ...
  const getLinkClass = (href: string) => {
    ...
    if (isActive(href)) {
      return `${baseClass} text-orange-500 text-base`
    }
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <Link href="/admin/login" className={getLinkClass('/admin')}>
              Admin
            </Link>
          </nav>
        ...
        {isMenuOpen && (
          <nav className="md:hidden pb-4 space-y-2">
            ...
            <Link href="/collections" className={`block py-2 ${getLinkClass('/collections')}`}>
              Collections
            </Link>
            ...
            <Link href="/admin/login" className={`block py-2 ${getLinkClass('/admin')}`}>
              Admin
            </Link>
          </nav>
        )}
